...
public class TrappingRainWater
{
    ...
    {
        int left = 0;                    // start from the leftmost bar
        int right = height.Length - 1;   // start from the rightmost bar

        int left_max = 0;   // tallest wall we've seen so far coming from the left
        int right_max = 0;  // tallest wall we've seen so far coming from the right
        ...
        while(left < right)
        {
            ...
            if(height[left] < height[right])
            {
                ...
                if(height[left] >= left_max)
                    // This bar is taller than anything we've seen from the left
                    // No water can sit here — it would just flow off
                    // Update our left_max instead
                    left_max = height[left];
                else
                    // This bar is shorter than left_max
                    // Water fills up to left_max level, so trapped water = difference
            ...
            else
            {
                ...
                if(height[right] >= right_max)
                    // This bar is taller than anything from the right — no water here
                    right_max = height[right];
                else
                    // Shorter bar — water fills the gap between right_max and this bar
